...
public class PlayerController : MonoBehaviour
{
    ...
    bool facingRight = true;//Indica hacia que lado está mirando el personaje
...
/*
   IEnumerator getColliderSwitcher(Collider2D StandCollider, Collider2D CrouchCollider)
   {
        if (StandCollider.enabled == true) 
        {
            switchColliderOff(StandCollider);
            switchColliderOn(CrouchCollider);

            yield return new WaitUntil(() => CrouchCollider.enabled == true);
        ...
        } else
        {
            ...
            yield return new WaitUntil(() => StandCollider.enabled == true);
...
*/
    ...
    void flip() //esta funcion transforma facing right a facing left
    {
